...
import { UtensilsCrossed, Plus, Edit, Trash2, Search, Image as ImageIcon, MapPin, Palmtree, Store, Wheat, Waves, Castle, Leaf, Flame, WheatOff } from 'lucide-react';
      ...
      {filteredItems.length === 0 ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
          {filteredItems.map((item) => (
            ...
            >
              {/* Image Section */}
              <div className="h-28 md:h-48 w-full bg-gray-100 relative shrink-0">
                {item.image ? (
                  <img 
                    src={item.image} 
                    alt={item.name} 
              ...
              <div className="p-3 md:p-5 flex flex-col flex-1">
                <div className="mb-auto">
                  <div className="w-full">
                    <h3 className="text-sm md:text-lg font-bold text-gray-900 truncate block w-full leading-tight" title={item.name}>
                      {item.name}
                    </h3>
                  </div>
                  ...
                  <div className="flex gap-1 mt-2 flex-wrap min-h-[20px]">
                    {item.isVegetarian && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-lime-100 text-lime-800 rounded border border-lime-200 shrink-0">Veg</span>
                    )}
                    {item.isGlutenFree && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-yellow-100 text-yellow-800 rounded border border-yellow-200 shrink-0">GF</span>
                    )}
                    {item.spicyLevel > 0 && (
                      <span className="text-[10px] px-1.5 py-0.5 bg-red-100 text-red-800 rounded border border-red-200 shrink-0">
                  ...
                  <p className="text-base md:text-xl font-bold text-orange-600 mt-2 md:mt-3">
                ...
                <div className="flex gap-2 mt-3 md:mt-5">
                  <button
                    ...
                    className="flex-1 h-[32px] md:h-[36px] bg-orange-50 text-orange-700 border border-orange-200 rounded-lg text-xs md:text-sm font-semibold hover:bg-orange-100 transition-colors flex items-center justify-center gap-1.5 shrink-0"
                  >
                  ...
                  <button
                    ...
                    className="h-[32px] md:h-[36px] w-[32px] md:w-[36px] bg-red-50 text-red-600 border border-red-100 rounded-lg hover:bg-red-100 transition-colors flex items-center justify-center shrink-0"
                  >
